...
std::optional<Screens> SandboxScreen::handle_input() {
    ...
    if (lclick && mouse.x < SB_PANEL_W) {
        ...
        for (int i = 0; i < (int)fixtures.size(); ++i) {
            if (mouse.y >= y && mouse.y < y + SB_ITEM_H) {
                if (i != fixture_idx) {
                    fixture_idx = i;
                    current_ms  = fixture_start_ms = get_current_ms();
                    fixtures[fixture_idx]->reset(current_ms);
        ...
        int  type_row_h   = SB_TYPE_H + 2;
        int  y_types_start = 46 + (int)fixtures.size() * SB_ITEM_H + 8;
        if (!type_list.empty()) {
            int ty = y_types_start;
        ...
        // Variant button
        int y_types_end = y_types_start + (int)type_list.size() * type_row_h;
        int var_btn_y   = y_types_end + (type_list.empty() ? 0 : 8);
        ...
        int btn_y = tex.screen_height - 16 - 4 - SB_BTN_H;
        if (mouse.x >= SB_BTN_MARG && mouse.x < SB_PANEL_W - SB_BTN_MARG &&
            mouse.y >= btn_y && mouse.y < btn_y + SB_BTN_H)
            paused = !paused;
    }
...
void SandboxScreen::draw_panel() const {
    ...
    for (int i = 0; i < (int)fixtures.size(); ++i) {
        ...
        bool hovered  = !selected && mouse.x < SB_PANEL_W && mouse.y >= y && mouse.y < y + SB_ITEM_H;
        if (selected)
            ray::DrawRectangle(0, y, SB_PANEL_W, SB_ITEM_H, ray::Color{60, 80, 180, 220});
        else if (hovered)
            ray::DrawRectangle(0, y, SB_PANEL_W, SB_ITEM_H, ray::Color{40, 50, 100, 160});
        ray::Color col = selected ? ray::WHITE : (hovered ? ray::Color{220, 220, 255, 255} : ray::Color{180, 180, 180, 255});
        ray::DrawText(fixtures[i]->name.c_str(), 10, y + 7, 16, col);
        y += SB_ITEM_H;
    }

    // Separator + type buttons
    int  y_list_end    = 46 + (int)fixtures.size() * SB_ITEM_H;
    ray::DrawLine(0, y_list_end + 4, SB_PANEL_W, y_list_end + 4, ray::Color{80, 80, 100, 255});

    auto type_list    = fixtures[fixture_idx]->type_names();
    int  type_row_h   = SB_TYPE_H + 2;
    int  y_types_start = y_list_end + 8;
    if (!type_list.empty()) {
        int ty = y_types_start;
        for (int i = 0; i < (int)type_list.size(); ++i) {
        ...
        int y_types_end_line = y_types_start + (int)type_list.size() * type_row_h;
        ray::DrawLine(0, y_types_end_line + 4, SB_PANEL_W, y_types_end_line + 4, ray::Color{80, 80, 100, 255});
    }

    // Variant button
    int y_types_end  = y_types_start + (int)type_list.size() * type_row_h;
    int  var_btn_y   = y_types_end + (type_list.empty() ? 0 : 8);
    bool var_btn_hov = mouse.x >= SB_BTN_MARG && mouse.x < SB_PANEL_W - SB_BTN_MARG &&
                       mouse.y >= var_btn_y && mouse.y < var_btn_y + SB_BTN_H;
    ray::DrawRectangle(SB_BTN_MARG, var_btn_y, SB_PANEL_W - SB_BTN_MARG * 2, SB_BTN_H,
                       ray::Color{80, 80, 160, static_cast<unsigned char>(var_btn_hov ? 255 : 200)});
    ...
    ray::DrawRectangle(SB_BTN_MARG, btn_y, SB_PANEL_W - SB_BTN_MARG * 2, SB_BTN_H, btn_col);
    const char* btn_label = paused ? "|| PAUSED" : ">  PLAYING";
    ray::DrawText(btn_label, SB_BTN_MARG + 6, btn_y + 7, 14, ray::WHITE);

...
void HighScoreIndicator::draw() {
    ...
    for (int i = 0; i < reversed_score.length(); ++i) {
        ...
        tex.draw_texture("score", "high_score_num", {.frame=digit, .x=-(i*tex.skin_config["high_score_indicator_margin"].x), .y=(float)move->attribute, .fade=fade->attribute, .index=is_2p});
    }
}

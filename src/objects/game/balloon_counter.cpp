...
void BalloonCounter::draw(float y) {
    ...
    if (balloon_count > 0) {
        ...
        for (int i = 0; i < counter.size(); i++) {
            ...
            tex.draw_texture("balloon", "counter", {.frame=digit - '0', .x=-(total_width / 2.0f) + (i * tex.skin_config["drumroll_counter_margin"].x), .y=y - (float)stretch->attribute, .y2=(float)stretch->attribute, .fade=fade->attribute});
        }
    }
}
